...
import { WorkflowStatusBadge, WorkflowPriorityBadge, StepStatusBadge, TaskStatusBadge, TaskPriorityBadge } from "@/components/ui/StatusBadge";
...
export default async function WorkflowShowPage({ params }: Props) {
   ...
   return (
      <div>
         <PageHeader title={workflow.title} subtitle="جزئیات گردش کار و مراحل مربوطه" icon="fa-solid fa-diagram-project" />
         ...
         <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div className="space-y-6 lg:col-span-1">
               <Card>
                  ...
                  <CardBody>
                     ...
                     <div className="mt-5 flex items-center gap-2 border-t border-slate-100 pt-4">
                        <Link href={`/workflows/${workflow.id}/edit`} className="inline-flex flex-1 items-center justify-center gap-2 rounded-lg bg-sky-50 px-4 py-2 text-sm font-medium text-sky-700 transition hover:bg-sky-100">
                           <i className="fa-solid fa-pen" />
                           ویرایش
                        </Link>
                        <DeleteButton id={workflow.id} entityName="گردش کار" endpoint="/api/workflows" redirectTo="/workflows" />
                     </div>
                  </CardBody>
               </Card>
               ...
               {workflow.steps.length > 0 && (
                  <Card>
                     <CardHeader title="مراحل گردش کار" subtitle={`${formatNumber(workflow.steps.length)} مرحله`} icon="fa-solid fa-list-ol" />
                           ...
                           <div className="space-y-4">
                              {workflow.steps.map((step, i) => (
                                 <div key={step.id} className="relative flex items-start gap-4">
                                    <span className="z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border-2 border-slate-200 bg-white text-xs font-bold text-slate-500">{formatNumber(i + 1)}</span>
            ...
            <div className="lg:col-span-2">
               <Card>
                  <CardHeader
                     ...
                     action={
                        <Link href="/tasks/new" className="inline-flex items-center gap-1.5 rounded-lg bg-slate-800 px-3 py-1.5 text-xs font-medium text-white transition hover:bg-slate-700">
