...
import { TaskStatusBadge, TaskPriorityBadge, StepStatusBadge } from "@/components/ui/StatusBadge";
...
export default async function TaskShowPage({ params }: Props) {
   ...
   return (
      <div>
         <PageHeader title={task.title} subtitle="جزئیات تسک و اطلاعات مرتبط" icon="fa-solid fa-list-check" />
         ...
         <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div className="space-y-6 lg:col-span-1">
               <Card>
                  ...
                  <CardBody>
                     ...
                     <div className="mt-5 flex items-center gap-2 border-t border-slate-100 pt-4">
                        <Link href={`/tasks/${task.id}/edit`} className="inline-flex flex-1 items-center justify-center gap-2 rounded-lg bg-sky-50 px-4 py-2 text-sm font-medium text-sky-700 transition hover:bg-sky-100">
                           <i className="fa-solid fa-pen" />
                           ویرایش
                        </Link>
                        <DeleteButton id={task.id} entityName="تسک" endpoint="/api/tasks" redirectTo="/tasks" />
                     </div>
                  </CardBody>
               </Card>
               ...
               {task.workflowStep && (
                  <Card>
                     <CardHeader title="مرحله فعلی گردش کار" subtitle="وضعیت مرحله مربوطه" icon="fa-solid fa-list-ol" />
                     <CardBody>
                        <div className="flex items-start gap-3">
                           ...
                           <div>
                              ...
                              {task.workflowStep.description && <p className="mt-1 text-xs text-slate-500">{task.workflowStep.description}</p>}
            ...
            <div className="lg:col-span-2">
               <Card>
                  <CardHeader title="نظرات" subtitle={`${formatNumber(task.comments.length)} نظر`} icon="fa-solid fa-comments" />
                  <CardBody>
                     {task.comments.length === 0 ? (
                        ...
                        <div className="space-y-4">
                           {task.comments.map((comment) => (
                              <div key={comment.id} className="rounded-xl border border-slate-100 bg-slate-50/70 p-4">
                                 <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                       <span className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-slate-500 to-slate-700 text-xs font-bold text-white">{comment.user?.name?.charAt(0) || "؟"}</span>
                                       <div>
