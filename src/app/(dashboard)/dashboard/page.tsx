...
export default async function DashboardPage() {
  ...
  const [totalDepartments, totalWorkflows, totalTasks, pendingTasks, inProgressTasks, completedTasks] =
    await Promise.all([
    ...
    <div className="space-y-6">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-slate-500">
          خوش آمدید، {session?.user.name} عزیز! نمای کلی سیستم گردش کار را مشاهده می‌کنید.
        </p>
      </div>
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {recentWorkflows.map((wf) => (
                      <tr key={wf.id} className="transition hover:bg-slate-50/70">
                        ...
                        <td className="py-3">
                          <Badge tone={wf.priority >= 3 ? "red" : wf.priority === 2 ? "blue" : "gray"}>
                            {wf.priority >= 3 ? "زیاد" : wf.priority === 2 ? "متوسط" : "کم"}
                          </Badge>
                        </td>
                        <td className="py-3">
